...
export default function Paradas() {
  ...
  const [previsoesPorParada, setPrevisoesPorParada] = useState<PrevisoesPorParada>({});
  ...
  const getParadasPorLinha = async () => {
    try {
      const response = await axios.get(`https://aiko-olhovivo-proxy.aikodigital.io/Parada/BuscarParadasPorLinha?codigoLinha=${params.cl}`);
  ...
  const getPosition = useCallback(async () => {
    try {
      const position = await axios.get(`https://aiko-olhovivo-proxy.aikodigital.io/Posicao/Linha?codigoLinha=${params.cl}`);
  ...
  const getPrevisaoChegada = useCallback(async (cp: number) => {   
    try {
      const response = await axios.get(`https://aiko-olhovivo-proxy.aikodigital.io/Previsao?codigoParada=${cp}&codigoLinha=${params.cl}`);
  ...
  return (
    ...
    <MapViewContainer
      ...
      mapRegion={mapRegion}
    />
    <BusPredictions selectedParada={selectedParada} previsoesPorParada={previsoesPorParada} position={position} />
  </View>
  );
}
